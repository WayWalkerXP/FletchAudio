...
class ValidationManager:
    ...
    def validate(self, source_info: AudioInfo, plan: ConversionPlan, output_path: Path) -> bool:
        ...
        if output_info.channels != plan.output_channels:
            logging.error("Validation failed; expected %s audio (%s channel(s)), found %s channel(s)", plan.output_channel_description, plan.output_channels, output_info.channels)
            return False
        ...
        if output_info.chapter_count != source_info.chapter_count:
            logging.error("Validation failed; source has %s chapter(s), output has %s chapter(s)", source_info.chapter_count, output_info.chapter_count)
            return False
