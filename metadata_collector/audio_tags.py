...
def _first(v):
    ...
    return v
def _int_part(v):
    try: return int(str(_first(v)).split('/')[0])
    except Exception: return None
def _text(tags,*keys):
    for k in keys:
        if tags and k in tags:
            v=_first(tags[k]); return str(v) if v is not None else None
    return None
def read_audio_metadata(path: str) -> AudioFileMetadata:
    ...
    if isinstance(audio, MP4):
        ...
        m.asin=_text(tags,'----:com.apple.iTunes:ASIN'); m.genres=list(tags.get('©gen',[]) or []); m.track=(tags.get('trkn') or [(None,None)])[0][0]; m.disc=(tags.get('disk') or [(None,None)])[0][0]
        ...
        g=_text(tags,'TCON'); m.genres=[g] if g else []; m.track=_int_part(_text(tags,'TRCK')); m.disc=_int_part(_text(tags,'TPOS')); m.has_cover=any(str(k).startswith('APIC') for k in tags.keys())
    return m
